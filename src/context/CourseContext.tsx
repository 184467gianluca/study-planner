...
export function CourseProvider({ children }: { children: ReactNode }) {
  const [savedCourses, setSavedCourses] = useLocalStorage<Course[]>(
    ...
    defaultMeteorologyCourses,
  );
  const [currentSemester, setCurrentSemester] = useLocalStorage<number>(
    "study-planner-current-semester",
    1,
  );
  const [startSeason, setStartSeason] = useLocalStorage<Season>(
    "study-planner-start-season",
    "WiSe",
  );
  ...
  const courses = savedCourses.map((course) => {
    ...
    // Phase 15.4: Container Module logic (status depends purely on accumulated CP)
    if (course.isContainer) {
      // Find all children that belong to this container
      const children = savedCourses.filter(
        (c) => c.parentModuleId === course.id,
      );

      // Tally CP of only the COMPLETED children
      const accumulatedCP = children.reduce((acc, child) => {
        const isChildPassed =
          !child.isGraded ||
          (child.isGraded && child.grade && child.grade <= 4.0);
        const isPast = child.semester < currentSemester;
        const isCurrentButPassed =
          child.semester === currentSemester && isChildPassed && child.grade;

        if ((isPast && isChildPassed) || isCurrentButPassed) {
          return acc + (child.credits || 0);
        }
        return acc;
      }, 0);

      if (accumulatedCP >= (course.credits || 0)) {
        status = "completed";
      } else if (accumulatedCP > 0) {
        status = "in-progress";
      } else {
        status = "planned"; // Default if 0 CP
      }
    ...
    const isPassed =
      !course.isGraded ||
      (course.isGraded && course.grade && course.grade <= 4.0);
  ...
  const updateCourse = (id: string, updatedFields: Partial<Course>) => {
    ...
    setSavedCourses((prev) => {
      ...
      return prev.map((course) => {
        ...
        if (
          isComposite &&
          course.compositeExamId === targetCourse.compositeExamId
        ) {
          return {
            ...course,
