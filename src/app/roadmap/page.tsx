...
export default function RoadmapView() {
  ...
  const getCategoryStyles = (category?: string) => {
    switch (category) {
      ...
      case "physics":
        return "bg-purple-500/5 text-purple-300 border-purple-500/20";
      case "math":
      ...
      default:
        return "bg-surface/50 text-foreground-muted border-border/50";
    }
  };

  // Phase 11 & 15: Dynamic Grouping based on View Mode and Container Spans
  const coursesBySemester = courses.reduce(
    (acc, course) => {
      // Multi-semester containers mapping
      if (
        course.isContainer &&
        course.containerSemesters &&
        course.containerSemesters.length > 0
      ) {
        course.containerSemesters.forEach((sem) => {
          if (!acc[sem]) acc[sem] = [];
          acc[sem].push(course);
        });
        return acc;
      ...
      // Hide containers without mapped semesters
      if (course.isContainer) return acc;

      const sem =
        viewMode === "official"
          ? course.recommendedSemester || course.semester
          : course.semester;
      if (!acc[sem]) acc[sem] = [];
      acc[sem].push(course);
      return acc;
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4 custom-scrollbar">
          <div className="flex gap-6 h-full items-start min-w-max pb-4 px-2">
            {activeSemesters.map((semesterNum) => {
              ...
              // Container shell "Target CP" is ignored, only pure course CP is summed for the semester total
              const totalCP = semesterCourses.reduce(
                (acc, c) => acc + (c.isContainer ? 0 : c.credits || 0),
                0,
              );
              ...
              return (
                <div
                  key={`sem-${semesterNum}`}
                  className="flex flex-col w-[320px] max-h-full shrink-0"
                >
                  {/* Column Header */}
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar pb-2">
                    {semesterCourses
                      // Render courses that either have no parent, OR their parent is not in this semester column
                      .filter(
                        (c) =>
                          !c.parentModuleId ||
                          !semesterCourses.some(
                            (p) => p.id === c.parentModuleId,
                          ),
                      )
                      .map((course) => {
                        const childCourses = semesterCourses.filter(
                          (c) => c.parentModuleId === course.id,
                        );

                        return (
                          <Card
                            key={`${course.id}-${semesterNum}`}
                            className={`border shadow-sm transition-all hover:bg-surface-hover ${getCategoryStyles(course.category)} relative overflow-hidden`}
                          >
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-2 text-[10px] opacity-80">
                                ...
                                {course.examType &&
                                  course.examType !== "none" && (
                                    <span
                                      className="flex items-center gap-1 bg-surface px-1.5 py-0.5 rounded border border-border"
                                      title={`${course.examType} Exam`}
                                    >
                              ...
                              {course.recommendedSemester &&
                                course.semester !==
                                  ...
                                  <div className="mt-2 text-[10px] font-medium">
                                    {viewMode === "custom" ? (
                                      <span
                                        className="inline-block px-1.5 py-0.5 rounded border border-orange-500/30 text-orange-400 bg-orange-500/10"
                                        title="Moved from the official curriculum plan"
                                      >
                                        PO: Sem {course.recommendedSemester}
                                      </span>
                                    ) : (
                                      <span
                                        className="inline-block px-1.5 py-0.5 rounded border border-border text-foreground-muted bg-surface"
                                        title="Currently planned in a different semester"
                                      >
