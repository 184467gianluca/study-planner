...
export default function CourseManager() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const courseData: Partial<Course> = {
      ...
      containerSemesters: formData.isContainer
        ? formData.containerSemesters
            .split(",")
      ...
      parentModuleId:
        formData.parentModuleId === "none"
          ? undefined
          : formData.parentModuleId,
      sws: formData.sws ? Number(formData.sws) : undefined,
      isGraded: formData.isGraded,
      grade:
        formData.isGraded && formData.grade
          ? Number(formData.grade)
          : undefined,
      isContainer: formData.isContainer,
      hasExercise: formData.hasExercise,
      exerciseSws:
        formData.hasExercise && formData.exerciseSws
          ? Number(formData.exerciseSws)
          : undefined,
      workloadMin:
        formData.hasExercise && formData.workloadMin
          ? Number(formData.workloadMin)
          : undefined,
      workloadMax:
        formData.hasExercise && formData.workloadMax
          ? Number(formData.workloadMax)
          : undefined,
      admissionStatus: formData.admissionStatus,
      examType: formData.examType,
      offeredIn: formData.offeredIn,
      examAttemptsMax:
        formData.examType !== "none" && formData.examAttemptsMax
          ? Number(formData.examAttemptsMax)
          : undefined,
      examAttemptsUsed:
        formData.examType !== "none" && formData.examAttemptsUsed
          ? Number(formData.examAttemptsUsed)
          : undefined,
    ...
    if (formData.compositeTargetId !== "none") {
      const targetCourse = courses.find(
        (c) => c.id === formData.compositeTargetId,
      );
      if (targetCourse) {
  ...
  const handleEditClick = (course: Course) => {
    ...
    setFormData({
      ...
      isContainer: course.isContainer || false,
      hasExercise: course.hasExercise || false,
      exerciseSws:
        course.exerciseSws !== undefined ? String(course.exerciseSws) : "",
      workloadMin:
        course.workloadMin !== undefined ? String(course.workloadMin) : "",
      workloadMax:
        course.workloadMax !== undefined ? String(course.workloadMax) : "",
      admissionStatus: course.admissionStatus || "not-required",
      examType: course.examType || "written",
      offeredIn: course.offeredIn || "both",
      examAttemptsMax:
        course.examAttemptsMax !== undefined
          ? String(course.examAttemptsMax)
          : "3",
      examAttemptsUsed:
        course.examAttemptsUsed !== undefined
          ? String(course.examAttemptsUsed)
          : "0",
      ...
      compositeTargetId: course.compositeExamId
        ? courses.find(
            (c) =>
              c.id !== course.id &&
              c.compositeExamId === course.compositeExamId,
          )?.id || "none"
        : "none",
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="mb-6 mb-8 border-primary/50 shadow-[0_0_15px_rgba(0,229,255,0.1)]">
        <CardHeader>
          <CardTitle className="text-secondary drop-shadow-[0_0_5px_rgba(0,255,157,0.4)]">
            Global Settings
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setCurrentSemester(Number(e.target.value) || 1)
                }
                className="mt-1"
              />
              <p className="text-xs text-foreground-muted mt-2">
                Course statuses (Completed, In Progress, Planned) are calculated
          ...
          <div className="mt-8 pt-6 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-foreground-muted">
                Export your current plan or load an existing one.
              </p>
            </div>
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <div className="relative flex-1 sm:flex-none">
                <Label
                  htmlFor="import-file"
                  className="flex items-center justify-center w-full h-10 px-4 py-2 text-sm font-medium transition-colors border rounded-md cursor-pointer border-border hover:bg-surface-hover hover:text-foreground"
                >
          ...
          <div className="mt-6 pt-6 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-foreground-muted">
                Revert mistakes or reset curriculum entirely.
              </p>
            </div>
            <div className="flex gap-3 w-full sm:w-auto">
              <Button
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    confirm(
                      "Are you sure you want to reset EVERYTHING to the default Meteorology curriculum? All manual changes, grades, and attempts will be lost forever.",
                    )
                  ) {
                    resetToDefaults();
                  }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex items-start gap-3 p-4 rounded-lg border border-primary/20 bg-primary/5 mb-4">
                ...
                <div>
                  ...
                  >
                    Multi-Semester Container Module
                  </Label>
                  <p className="text-xs text-foreground-muted mt-0.5">
                    Check this if this course is a "pool" meant to collect CP
                    from smaller sub-courses. Child modules can select this as
                    their parent.
                  </p>
                </div>
              </div>
                ...
                <Input
                  ...
                  placeholder={
                    formData.isContainer
                      ? "e.g. Wahlpflichtbereich"
                      : "e.g. Advanced Calculus"
                  }
                  required
                />
              </div>
                ...
                {!formData.isContainer && (
                  <div>
                    ...
                    <Input
                      ...
                      value={formData.sws}
                      onChange={(e) =>
                        setFormData({ ...formData, sws: e.target.value })
                      }
                      placeholder="e.g. 4"
                    />
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {!formData.isContainer ? (
                  ...
                  <div>
                    <Label htmlFor="containerSemesters">
                      Active Semesters (Comma Separated)
                    </Label>
                    <Input
                      id="containerSemesters"
                      type="text"
                      value={formData.containerSemesters}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          containerSemesters: e.target.value,
                        })
                      }
                      placeholder="e.g. 3, 5"
                    />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        offeredIn: e.target.value as "WiSe" | "SoSe" | "both",
                      })
                    }
                    className="mt-1 flex h-10 w-full rounded-md border border-border bg-surface px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-colors"
                  >
              ...
              <div className="pt-4 border-t border-border mt-4">
                ...
                {formData.hasExercise && (
                  <div className="space-y-4 bg-surface-hover/30 p-3 rounded-md border border-border/50">
                    <div>
                      <Label htmlFor="exerciseSws">
                        Exercise SWS (optional)
                      </Label>
                      <Input
                        id="exerciseSws"
                        type="number"
                    ...
                    <div>
                      <Label htmlFor="admissionStatus">
                        Exam Admission (Zulassung)
                      </Label>
                      <select
                        id="admissionStatus"
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            admissionStatus: e.target.value as
                              | "not-required"
                              | "pending"
                              | "granted",
                          })
                      ...
                      >
                        ...
                        <option value="pending">
                          Pending (Working on Sheets)
                        </option>
                        <option value="granted">
                          Granted (Zulassung erhalten!)
                        </option>
                      </select>
                    </div>
                  </div>
              ...
              <div className="pt-4 border-t border-border mt-4">
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        examType: e.target.value as "written" | "oral" | "none",
                      })
                    }
                  ...
                  >
                    <option value="none">
                      None (Portfolioprüfung/Presentation)
                    </option>
                ...
                <div className="mb-4">
                  <Label htmlFor="compositeTargetId">
                    Part of Composite Exam with... (Doppelklausur)
                  </Label>
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        compositeTargetId: e.target.value,
                      })
                    }
                    className="mt-1 flex h-10 w-full rounded-md border border-border bg-surface px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-colors"
                  >
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            examAttemptsMax: e.target.value,
                          })
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            examAttemptsUsed: e.target.value,
                          })
                        }
                      />
                    </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, isGraded: e.target.checked })
                      }
                      className="w-4 h-4 text-primary bg-surface border-border rounded focus:ring-primary focus:ring-2"
                    />
                  </div>
                ...
                {formData.isGraded && (
                  <div className="flex items-center justify-between mb-4">
                    <Label
                      htmlFor="countsTowardsFinalGrade"
                      className="font-medium text-secondary"
                    >
                      Counts towards B.Sc. Final Grade?
                    </Label>
                    <div className="flex items-center">
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            countsTowardsFinalGrade: e.target.checked,
                          })
                        }
                ...
                {formData.isGraded && (
                  <div>
                    ...
                    <p className="text-[10px] text-foreground-muted mt-1">
                      Grades are used to calculate your average current grade.
                    </p>
                  </div>
                )}
              </div>
